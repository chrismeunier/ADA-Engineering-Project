...
sources_rdd = sc.textFile(SOURCES_FILE).map(lambda r: r.split('\t'))

#create schema and change date type for date column
...
avg_countryTopics = country_dist.groupby("country").mean().drop("avg(textID)")
...
text_US_topic0 = distribution.join(text_US_topic0, distribution.textID == text_US_topic0.d_textID).drop("d_textID").sort("topic[8]",ascending=False).limit(1000)
text_US_topic0 = text_US_topic0.select(col("textID"), col("date"))

wlp_rdd = sc.textFile('/datasets/now_corpus/corpus/text/*-*-us.txt') #first read the text file
...
############## Saving stage ##############
#renaming columns because parquet doesn't like [] nor ()
...
#save us sentiment analysis
sentiment_US_0.write.mode('overwrite').parquet('sentiment_US_0.parquet')
